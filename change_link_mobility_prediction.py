...
def get_trace(node, file):
    ...
    #         trace = trace_node.get_group(n)
            # for row in trace:
    for line in range(1, len(df_trace)):
        t = float(df_trace.loc[line - 1, "time"])
        t_1 = float(df_trace.loc[line, "time"])
        if line == 1:
            state.append(int(float(df_trace.loc[line-1, "state"])))
            state_interval.append(t_1 - t)
        state.append(int(float(df_trace.loc[line, "state"])))
        state_interval.append(t_1 - t)
...
if __name__ == '__main__':
    ...
    if args.eventFile == '':
        if args.interface and args.latency and args.dest and args.src and args.traceFile and args.qlen:
            states, state_interval = get_trace(0, args.traceFile)
        ...
        if args.interface and args.latency and args.dest and args.src and args.traceFile and args.traceFile2 and args.qlen and args.eventFile:
            states, state_interval = get_trace(0, args.traceFile)
            states2, state_interval2 = get_trace(0, args.traceFile2)
