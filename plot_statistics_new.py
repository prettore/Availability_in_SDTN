...
def main2(data_path, show, noolsr):
    ...
    if os.path.isfile(send_signal_file):
...
def main(data_path, show, noolsr):
    ...
    if short:
        ...
        fig, ax = plt.subplots(4, 1, sharex='col', figsize=(6, 3), gridspec_kw={'hspace': 0, 'wspace': 0},tight_layout=True)
    else:
        # create a group of prot, and remove vertical gap between subplots
        fig, ax = plt.subplots(6, 1, sharex='col', figsize=(6, 5), gridspec_kw={'hspace': 0.0, 'wspace': 0},tight_layout=True)
    ...
    line2 = ax[2].plot(df_sender_buffer['buffer_timestamp'], df_sender_buffer['pr4g_queue_occupancy'], linewidth=1.5, label='Buffer occupancy', color='tab:blue')
    ax[2].set_ylabel("Buffer\n(%)")
    ax[2].grid(color='gray', linestyle='dashed', lw = 0.5)
    yticks = ax[2].yaxis.get_major_ticks()
    yticks[-1].label1.set_visible(False)

    line3 = ax[3].plot(df_time_series['time'], df_time_series['packet_loss'], linewidth=1.5, label='Packet loss',color='tab:orange')
    ...
    if not short:
        line4 = ax[4].plot(df_time_series['time'], df_time_series['latency'], linewidth=1.5,color='tab:orange')#, label='Latency')
        ...
        line5 = ax[5].plot(df_time_series['time'], df_time_series['jitter'], linewidth=1.5,color='tab:orange', label='Jitter')
    ...
    if show:
        ...
        if not noolsr:
            plt.savefig(path + 'pendulum_olsr.pdf')#, bbox_inches='tight', dpi=200)
        else:
            plt.savefig(path + 'pendulum_no-olsr.pdf')#, bbox_inches='tight', dpi=200)
